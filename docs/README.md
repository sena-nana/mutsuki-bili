...
| 米画师分享卡片 | QQ 转发链接卡片、小程序分享卡片中嵌入的米画师画师页 / 橱窗页 |

同一链接在同一频道内有冷却时间（默认 1 分钟），避免重复解析。单条消息最多解析 3 个链接。

卡片解析会尝试展开 JSON/XML/CQ 码中的 `jumpUrl`、`qqdocurl`、`pagepath` 等字段，并支持从 B 站小程序路径中的 `bvid`、`avid`、动态 ID、直播间 ID，以及米画师小程序路径中的画师 / 橱窗 ID 派生出可解析链接。
